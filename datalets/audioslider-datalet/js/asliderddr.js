var aslider = {
    ...
    initAsliders: function (sliders) {
        ...
        for (var i = 0; i < sliders.length; i++) {
            ...
            nextButton.setAttribute('style', aslider.nextButtonStyle);
            nextButton.setAttribute('data-state', 'play');
            nextButton.setAttribute('onclick', 'aslider.nextSlide(' + sliderIndex + ')');

            var nextIcon = document.createElement('img');
            nextIcon.setAttribute('src', aslider.playIcon);
            nextIcon.setAttribute('style', 'width: inherit; height: inherit');
            nextButton.appendChild(nextIcon);
            currentSlider.appendChild(nextButton);

            var prevButton = document.createElement('a');

            prevButton.setAttribute('style', aslider.prevButtonStyle);
            prevButton.setAttribute('data-state', 'play');
            prevButton.setAttribute('onclick', 'aslider.prevSlide(' + (sliderIndex) + ')');

            var prevIcon = document.createElement('img');
            prevIcon.setAttribute('src', aslider.playIcon);
            prevIcon.setAttribute('style', 'width: inherit; height: inherit; transform: rotate(180deg);');
            prevButton.appendChild(prevIcon);
            currentSlider.appendChild(prevButton);
            /*ddr*/
        }
    },

    nextSlide: function (sliderIndex) {
        'use strict';
        console.log('nextSlide  ', sliderIndex)
        var currentSlide = aslider.sliders[sliderIndex].currentSlide;
    ...
    prevSlide: function (sliderIndex) {
        'use strict';
        console.log('prevSlide', sliderIndex)
        var currentSlide = aslider.sliders[sliderIndex].currentSlide;
    ...
    muteIconStyle: "display: block; height: 25px; width: 25px; position: absolute; left: 30px; top: 30px; cursor: pointer;",
    playPauseIconStyle: "display: block; height: 25px; width: 25px; position: absolute; left: 75px; top: 30px; cursor: pointer;",
    nextButtonStyle: "display: block; height: 25px; width: 25px; position: absolute; right: 30px; top: 30px; cursor: pointer;",
    prevButtonStyle: "display: block; height: 25px; width: 25px; position: absolute; right: 75px; top: 30px; cursor: pointer;",
